...
### 概念
传输层最重要的就是理解端口的概念
IP地址 + 端口号 = 网络中一台主机上的一个特定的进程
每个端口号在两个主机上是相互独立的
套接字（Socket） = ip地址：端口号
0-1023端口一般不使用（开发的时候）因为其是熟知端口号，给每台电脑上几乎都有的应用程序使用
TCP：有连接的传输服务 --->开销大，延迟高，但是可靠
UDP：无连接的传输服务 --->开销小，延迟低，但是不可靠
...
#### TCP报文
**源端口和目的端口（Source Port / Destination Port）** ： 2B，表示发送方和接收方的端口号
**序号（Sequence Number，SEQ）** ： 4B，这报文发送的数据的第一个字节的序号
**确认号（Acknowledgment Number，ack）** ： 4B，期望收到对方下一个报文段的第一个数据字节的序号（N表示N-1已收到）
**数据偏移（Data Offset）** ： 4b，指出TCP报文段的数据开始距离TCP报文的开始多远
**保留（Reserved）** ： 6b，保留作为以后使用，现在为0
**紧急位（URG）** ： 告诉系统这报文中有紧急数据
**确认位（ACK）** ： 仅当确认位为1时确认字段有效
**推送位（PSH）** ： 发送方为1的时候催促接收方尽快接受应用进程，不用缓存满了再发
**复位位（RST）** ： 为1表示出现严重差错
**同步位（SYN）** ： 为1时表示这是一个链接请求报文
**终止位（FIN）** ： 用来释放链接
**窗口（Window Size）** ： 2B，接收方目前允许发送方发送的数据量
**检验和（Checksum）** ： 2B，和UDP差不多，只需将UDP的17改为6
**紧急指针（Urgent Pointer）** ： 2B，仅在紧急位为1的时候有效，指向紧急数据的字节数
**选项（Options）** ： 不定长，MSS是TCP报文段中的数据字段的最大长度
**填充（Padding）** ： 为了让首部为4B的整数倍

#### TCP连接管理
??TCP连接的端口是套接字（Socket）
同一个IP地址可以有多个不同的TCP连接，同一个端口号也可以出现在多个不同的TCP链接中
...
- 1. SYN = 1 : 表示这是一个链接请求报文， seq = x ： client选择了一个初始序号x
- 2. SYN = 1 ： 表示server也向client发送了一个链接请求报文。 seq = y ： server选了了一个自己的初始序号y， ACK = 1 ： 确认位置为一， ack = x + 1 ： 期望收到client下一个报文段的第一个数据字节的序号
- 3. ACK = 1 ： 确认位 置为1， seq = x + 1 ： client的报文发送的第一个字节的序号往后推一位（因为第一次client握手的时候，要消耗掉一个序号）， ack = y + 1（因为第二次握手的时候server也消耗了一个序号）

??第三次握手可以消耗序号也可以不消耗，主要看第三次握手携不携带信息
...
- 1. FIN = 1 ： 终止位置为1，表示client要停止传信息了； seq = u ： 表示停止传信息时，client前面传过的最后一个序号 + 1；（FIN报文即使不携带数据，也要消耗一个序号）
- 2. ACK = 1 ： 表示确认位有效； seq = v ： 表示停止传信息时，server前面传过的最后一个序号； ack = u + 1 ： 表示期望收到下一个序号变为u + 1
- 3. FIN = 1 ： 终止位置置为1，表示server也要停止传输信息了； ACK = 1 ： 表示ack有效； seq = w ： 表示server前面传输的最后一个序号（因为半关闭状态的服务器可能又发送了一些信息）； ack = u + 1 ： 期望收到的下一个序号保持一样
- 4. ACK = 1 ： 确认位有效； ack = w + 1 ： client期望收到下一个序号变为w + 1； seq = u + 1 ： 停止传信息的时候，client传过的最后一个序号
